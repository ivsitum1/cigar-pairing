...
- [ ] `--check` varijanta (CI-safe, ne piše)
...
- [ ] `scripts/derive-drink-display-names.py` po uzoru na
...
- [ ] Ponovi `TAIL` regex iz `derive-drink-brands.py` (jakost, volumen, poklon
      pakiranje) + dekodiraj HTML entitete (`&#039;` → `'`)
- [ ] `--check` varijanta i **dodaj je u `.github/workflows/ci.yml`** uz ostale
      python gateove
- [ ] `lib/drinkName.ts`: `drinkNameLoc` čita prikazno ime kad postoji
- [ ] Test: nijedno prikazno ime ne sadrži `% Vol`, `u poklon kutiji`, `&#`
- [ ] Provjera: `raw_scrape_tails` = 0
- [ ] **Regresija koju moraš provjeriti:** `npx vitest run src/lib/drinkBuyLink.test.ts`
