# -*- coding: utf-8 -*-
"""Build Tequila_Kolekcija_Checklist.xlsx from tequila_catalog_raw.json + seed.

Pokretanje: python scripts/build-tequila-excel.py
"""
...
RAW = Path(__file__).resolve().parent / "output" / "tequila_catalog_raw.json"
SEED = Path(__file__).resolve().parent / "seed" / "tequilas_seed.json"
APP_JSON = ROOT / "src" / "data" / "tequilas.json"
...
def enrich_row(item: dict, seed: dict | None) -> dict:
    ...
    if not note:
        note = f"Heuristika — {style}, {age_tier}"
    return {
        "name": name,
        "quality": quality,
        "style": style,
        ...
        "url": item.get("url", ""),
        "note": note,
        "pairable": is_pairable(name, style, category, quality),
        "seed": seed is not None,
        "status": (seed or {}).get("status") or "",
    }
...
def write_workbook(catalog: list[dict], seeds: dict[str, dict]) -> None:
    ...
    ws_k.append(["Katalog tequila/mezcal — allez.hr + ecuga.com"])
    ...
    ws_s = wb.create_sheet("Sve tequile (rang)")
    ws_s.append(["SVE TEQUILE RANGIRANE po kvaliteti (sipping uz cigaru) | allez.hr + ecuga.com"])
    ws_s.append(["#", "Tequila", "Kval /10", "Regija", "Bilješka", "Cijena", "Shop"])
    ...
    ws_m.append([
        "Tequila", "Kval /10", "Regija", "Kategorija", "Age tier",
    ...
    ws_t.append(["Tequila", "Kval /10", "Age tier", "Cijena €", "Ducan/izvor", "Status / biljeska"])
    ...
    ws_sv.append(["SERVIRANJE + CIGARE — profili stilova za tequila"])
    ws_sv.append(["Profil / primjer", "Neat", "Rocks", "Best", "Cigar hint"])
    style_header(ws_sv, 2, 5)
    serve_map = {3: "++", 2: "+", 1: "~", 0: "x"}
    for label, style in [
        ("Blanco (Don Julio / Patrón Silver)", "blanco"),
        ("Reposado (Herradura / Casamigos)", "reposado"),
        ("Añejo (Don Julio / Patrón)", "anejo"),
        ("Extra Añejo (1942)", "extra-anejo"),
        ("Mezcal sipping", "mezcal"),
    ]:
        s = serving_for_style(style)
        ws_sv.append([
            label,
            serve_map.get(s.get("neat", 3), "++"),
            serve_map.get(s.get("rocks", 2), "+"),
            s["best"],
            cigar_hint_for_style(style),
        ])
    for row in master_rows[:40]:
        s = serving_for_style(row["style"])
        ws_sv.append([
            row["name"][:50],
            serve_map.get(s.get("neat", 3), "++"),
            serve_map.get(s.get("rocks", 2), "+"),
    ...
    for line in [
        "TEQUILA VODIČ — sipping uz cigaru",
        "",
        ...
        "Mixto i aromatizirani izrazi: Katalog OK, NE MASTER/app.",
        "Mezcal u MASTER samo ako quality >= 7 (jasan sipping profil).",
        "",
        "Pipeline: scrape-tequila-catalog.py → build-tequila-excel.py → [MASTER] → excel-to-tequila-json.py",
        "Izvori: allez.hr/shop/tequila-mezcal + ecuga.com",
    ]:
        ws_v.append([line])
...
def main() -> int:
    if not RAW.exists():
        print(f"Missing {RAW} — run scrape-tequila-catalog.py first")
        return 1
